...
module.exports = class DataIntegrator {
    ...
    */
    ...
    if (!playerActive){
      return {
        active: false,
        image: "SPOTIFY",
        bluetoothDevice: data.bluetooth.deviceAlias ? data.bluetooth.deviceAlias : null
      };
    }
  ...
  getImage(data) {
    //console.log(data)
    if (data.bluetooth.connected && data.bluetooth.active) {
      if ((data.bluetooth.trackname !== null) && data.spotify.trackName === data.bluetooth.trackName && data.spotify.imgUrl) {
        return data.spotify.imgUrl;
      } else {
        return "BLUETOOTH";
      }
    } else {
      if (data.spotify.active && data.spotify.imgUrl) {
        return data.spotify.imgUrl;
      } else {
        return "SPOTIFY";
      }
