...
const DomFactory = require("./js/DomFactory.js")
//const config = require('getconfig');

const {ipcRenderer} = require('electron')

const contentWrapperNode = document.getElementById('wrapper');
const contentFactory = new DomFactory(".");
...
const [binMap, edgeMap] = minSizeBinning(coords, config.drawWave ? 1 : config.minBinWidth);

let playerState, playerNode, oldPlayerNode;
...
function drawVisualizer() {
  ...
  if (rawData !== undefined) {
    ...
    if (config.drawWave === true){
      drawWave(edgeMap, binHeights);
    } else {
      drawBars(edgeMap, binHeights);
    }
...
function drawWave(edgeMap, binHeights){
  let edge = 0;
  let splinePoints = new Array(edgeMap.length);
  for (let i = 1; i < edgeMap.length; i++){

    edge = edgeMap[i] - edgeMap[1];
    let barHeight = (Math.log(binHeights[i]) - config.dampingFactor) * config.scalingFactor;

    barHeight = canvas.height - barHeight;
    splinePoints[i * 2] = edge;
    splinePoints[(i * 2) + 1] = barHeight;

  }
  canvasContext.strokeStyle = 'rgb(100,100,100)';
  canvasContext.lineWidth = 3;
  canvasContext.beginPath();
  canvasContext.moveTo(splinePoints[0], splinePoints[1])
  canvasContext.curve(splinePoints, 0.5)
  canvasContext.stroke();
  
}
function drawBars(edgeMap, binHeights) {
  let oldEdge;
  let edge = 0;
...
async function drawPlayer() { 
  playerNode = await contentFactory.buildDom(playerState); // wait for appropriate image to load before appending to dom
  if (!contentWrapperNode.hasChildNodes()){
    contentWrapperNode.appendChild(playerNode);
  } else {
    contentWrapperNode.replaceChild(playerNode, oldPlayerNode);
  }
  oldPlayerNode = playerNode;
}
process.stdout.on('data', function(data) { console.error("printed")})
try {
ipcRenderer.on('playerState', (event, state) => {
